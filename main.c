...
void digitalWrite(uint32_t pin_number, uint32_t value){
...
void setPORTD(unsigned char value){
    uint32_t pin;
    for (pin = 12;pin<20;pin++){
        nrf_gpio_pin_write(pin,(value & (1 << (pin-12)))>>(pin-12));
    }
}
...
void setup()                                       // for Arduino, runs first at power on
{
    ...
    setPORTDoutput(); //DDRD = 0xFF;                                    // configure PORTD as output
    ...
    switch(interface)
    {
        ...
        case 2:
            ...
            setPORTD(0); //PORTD = 0x00;                               // reset SDI_PIN and SCL_PIN, ground DB[5..0] of the display
